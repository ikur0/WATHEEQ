...
import glob
import json
from rag4 import ComplianceRAG 
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            # Question 1
            print("\nQ: want to see the retrieved context ?")
            print("(1) yes")
            print("(2) No")
            ...
            if show_ctx == '1':
                ...
                print("-" * 60)
                print(context.strip() if context.strip() else "(No context found)")
                print("="*60)
        ...
        # ---------------------------------------------
            ...
            if show_ctx == '1':
                ...
                raw_log = retrieval_result.get("raw_retrieval_results", {})
                for section_name, data in raw_log.items():
                    print(f"\n\n{'#'*60}")
                    print(f" {section_name.replace('_', ' ').upper()}")
                    print(f"{'#'*60}")
                    
                    print("\n[ COMPANY TEXT CHUNK (THE QUERY) ]")
                    print("-" * 60)
                    print(data.get("query", "").strip())
                    
                    print("\n[ RETRIEVED FRAMEWORK CONTEXT ]")
                    print("-" * 60)
                    ctx = data.get("context", "").strip()
                    print(ctx if ctx else "(No context retrieved for this section)")
