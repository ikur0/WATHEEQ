...
def initialize_standards_db():
    ...
    if not os.path.exists(file_path):
        st.error(f"❌ Critical Error: '{file_path}' not found in directory. Please add the Standards PDF.")
        return None
...
if "standards_db" not in st.session_state:
    with st.spinner("🔄 Loading National Standards Framework..."):
...
def generate_compliance_report(policy_text, is_full_report):
    ...
    # Call LLM
    with st.spinner("🔍 Comparing User Policy against National Standards..."):
...
if st.session_state.standards_db:
    st.success("✅ National Standards (file.pdf) Loaded & Active")
